...
void wrap_prod_complex(py::module &mod){
  using s = ctl::Product_complex< ctl::Simplex_boundary, ctl::Simplex_boundary>;
 
  typedef typename s::Cell c;
  py::class_<s>(mod, std::string("Prod_simplicial_complex").c_str())
